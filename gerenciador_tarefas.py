...
class GerenciadorTarefa:
            ...
            with open('Tarefas.txt', 'r', encoding='utf-8') as arquivo:
                for linha in arquivo:
                    if linha.startswith('Tarefa'):
                        ...
                        descricao = partes[1].strip() if len(partes) > 2 else None
                        status = partes[2].strip() if len(partes) > 2 else 'Pendente'
    ...
    def adicionar_tarefa(self):
        ...
        while True:
            self.linhas()
            nome_tarefa = input(f'Qual tarefa você vai adicionar a lista: ').strip().lower()
            if nome_tarefa != 'nenhuma':
                ...
                if nome_tarefa  not in[tarefa.nome for tarefa in self.lista_tarefas]:
                    ...
                    with open('Tarefas.txt', 'a', encoding='utf-8') as arquivo:
                        arquivo.write(f'Tarefa: {nova_tarefa.nome} - {nova_tarefa.descricao} - {nova_tarefa.status}\n')
                else:
    ...
    def exibir_tarefas(self):
        ...
        if not self.lista_tarefas:
            ...
            for i, tarefa in enumerate(self.lista_tarefas, start=1):
                ...
                print(f'{i} - {tarefa.nome} {descricao} - Status: {tarefa.status}') 
        self.linhas()
            ...
            for tarefa in self.lista_tarefas:
                if tarefa.nome.lower() == tarefa_concluida:
                    if tarefa.concluida:
                        ...
                        print(f'Tarefa {tarefa_concluida} já está marcada como concluída. ')
                        return
